...
If the app you're developing needs another container in the pod, such as a Postgres database, Redis, or RabbitMQ, list it under `k8s.sidecars`. Each entry runs alongside the coding container in the same pod and shares its network namespace, so the app in the coding container reaches it on `localhost`:
...
- **Single replica.** `k8s.replicas` must stay `1`: replicas share the same NAS folder, so two stateful sidecars would clobber each other (validated whenever `k8s.sidecars` is set, which defaults to `[]`, so effectively always `1`).
