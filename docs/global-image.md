...
- After any `system create` or `rollback`, existing projects pick the new version up on their next `deploy`/`create`, with no config changes and no re-builds on the projects' side.
