...
The entry points at the Kubernetes node's IP + the project's NodePort, so it works without any port-forward running. Pod host keys are ephemeral (regenerated on every image build), so the alias never stores them (`UserKnownHostsFile` points at the OS null device), so a rebuild can never leave a stale key behind to break VS Code with `Port forwarding is disabled`. `ssh`, `deploy`, and `delete` also remove any legacy `<project>-known_hosts` file left over from older versions.
