...
How `coding-container` works: a Node.js CLI that turns one YAML config into a running, SSH-accessible coding pod on Kubernetes, with the workspace persisted on NFS.
...
| Orchestration | Kubernetes via `kubectl` (manifests rendered in-process, applied from stdin) |
...
1. Hardcoded `DEFAULTS` (generic; environment-specific values like registry, NFS server, and NFS base path are intentionally left unset and must come from the global config)
2. **Global config:** `~/.config/coding-container/config.yaml` (honors `XDG_CONFIG_HOME`), if present. Holds the shared environment settings (registry, image build, k8s, NFS, SSH).
...
- Docker itself is spawned through `dockerExec()` (`src/utils/exec.js`): the `DOCKER_BIN` env var points at a specific binary/wrapper (tests use a stub); on Windows an overridden `DOCKER_BIN` is spawned with a shell (`shell: true`).
...
`src/utils/registry-api.js` is a small Docker Registry HTTP API V2 client: `listTags()` calls `GET /v2/<repo>/tags/list` (following the `Link: rel="next"` pagination header) and `deleteTag()` calls `DELETE /v2/<repo>/tags/<tag>`, where `<repo>` is the registry reference minus the host plus the image name (`registryRepoName()`). It performs the standard `WWW-Authenticate` dance. On a `401` it follows the challenge: `Basic` uses the local docker credentials directly as the `Authorization` header, `Bearer` exchanges them for a token (cached per realm/service/scope, probing `GET /v2/` first when no challenge is present). Requests go through core `node:http`/`node:https` with `keepAlive` disabled (not the global `fetch`) so the sockets never outlive the command and the CLI exits promptly. Deleting a tag reclaims disk only once the registry runs its own GC (automatic on GitLab/Harbor); AWS ECR does not expose tag deletion via the V2 API, so `gc` reports a `405` hint rather than pruning ECR.

### 2c. The shared global image (`src/utils/system-config.js`, `src/commands/system.js`)

The team's shared image state lives **in the cluster**, not on laptops: one ConfigMap, `coding-system` (label `coding-container=true`), in `k8s.namespace` (default `coding`). Its `data` block (all strings, since ConfigMaps carry strings) holds the version pointer and policy:
...
- **`system rollback [tag]`:** repoints `current` to an existing version (default: `previous`; the target must exist in the registry). Pure pointer change, no build, push or delete; `maxEver` never moves, and `previous` becomes the old `current`.
...
`deploy()` first resolves the image tag (`resolveDeployTag()`). Custom mode uses the project pin (required), while global mode uses the pin/`--tag` when set, else the `coding-system` ConfigMap's `current` (the float; a fresh cluster gets the `system init` hint). It then performs seven steps:
...
5. **Manifest apply:** `fullManifest()` (`src/utils/manifest.js`) renders up to five documents as YAML and pipes them to `kubectl apply -f -`:
    ...
    - **Service** `<project>`: `NodePort` (default, port `ssh.port` → `k8s.nodePort`, defaults `22` → `30022`) or `LoadBalancer`, plus a cluster port per `web.ports` entry (carrying a `nodePort` when the entry sets one), and a NodePort port per sidecar port carrying a `nodePort`.
   - **Ingress** `<project>`: rendered only when `web.ports` is non-empty. One rule per host: each entry's `host`/`hosts` routes to that entry's container port on the service, using `ingress.className` (default `traefik`). This is how browsers reach dev servers; entries with a `nodePort` are additionally reachable on every cluster node at `http://<node-ip>:<nodePort>/`.
6. **Rollout:** waits on `kubectl rollout status deployment/<project> --timeout=300s` (skippable with `deploy --no-wait`).
7. **SSH alias:** `addHostEntry()` (`src/utils/ssh-config.js`) writes/updates a `Host coding-<project>` block in the user's `~/.ssh/config`, in a marker-delimited managed section (everything outside the markers is never touched; a failure here only warns). The block points at the node's InternalIP + the project's NodePort, carries `StrictHostKeyChecking no` and a null-device `UserKnownHostsFile` (see §5), so `ssh coding-<project>` and VS Code Remote-SSH work without a port-forward. `delete` removes the block (`removeHostEntry()`).

### 4. The pod

The pod spec (rendered by `deploymentManifest()`, `src/utils/manifest.js`) declares two volumes:
...
- **Host keys:** pod sshd host keys are regenerated on every image build, so persisting them is pointless (a stale key makes VS Code Remote-SSH refuse forwarding with "Port forwarding is disabled"). The CLI connects with `StrictHostKeyChecking=no` and `UserKnownHostsFile` pointing at the OS null device (`NUL` on Windows, `/dev/null` elsewhere — `ephemeralKnownHostsFile()`, `src/utils/ssh-config.js`), so keys are never written to the user's global `~/.ssh/known_hosts` and a rebuilt image never blocks the next connection. `ssh`, `deploy` and `delete` also remove any legacy `~/.config/coding-container/keys/<project>-known_hosts` file left over from older versions (`removeLegacyKnownHostsFile()`). This is acceptable for development pods on a trusted network; the private key remains the only credential.
...
### 5b. SSH alias inventory (`src/commands/list.js`)

`deploy`/`create` register one `Host coding-<project>` block per project (see step 7 above). `list` is the read-only inventory of those managed blocks: it parses `~/.ssh/config` (`listHostEntries()`, `src/utils/ssh-config.js`) and prints a table of alias/host/port/user/identity file, sorted by host (IPv4-aware) then port. It never touches the cluster, config layers, or the SSH config.

`list update [--dry-run]` repairs stale aliases: it picks the IP of a currently-Ready node (`getReadyNodeIp()`, falling back to any addressed node when no Ready condition is present) and looks up each `coding-<project>` alias's service by name across all namespaces (`indexServicesByName()`), then rewrites the block's `HostName` and `Port` (from the service's `ssh` nodePort) via `addHostEntry()`. Stale entries (no matching service) and ambiguous ones (same service name in several namespaces) are left untouched with a warning, and nothing outside the managed block is modified. Both commands go through `kubectlExec()` (`KUBECTL_BIN` override, as everywhere else).

## Failure behaviour & exit codes

- Any step failure throws; `program.parseAsync(...).catch()` in `src/index.js` prints `Error: <message>` and exits with code 1.
...
For private, internal Kubernetes clusters only. Do not expose to the public web. This tool prioritizes development convenience over hardening: NodePort Services listen on every node, Ingress hosts have no authentication or TLS by default, pods run SSH as root, host-key checking is disabled, and Secrets are base64-encoded (not encrypted) unless you enable etcd encryption at rest. Only run it on a trusted internal network behind a firewall/VPN.

- The private key never leaves the developer machine; only the public key reaches the cluster (ConfigMap, namespace-scoped).
- `k8s.env` values are plaintext in the Deployment spec (visible to anyone who can `kubectl get deploy`). Put anything secret in `k8s.secretEnv` instead: values travel in the `<project>-env` Secret and the Deployment only carries a `secretKeyRef`. (A k8s Secret is base64, not encrypted, unless etcd encryption at rest is enabled. It still separates RBAC and stays out of deploy output. `validate --manifest` redacts secret values for the same reason.)
- Password authentication is disabled in the container; only key auth works. Pods still run SSH as root with host-key checking disabled (`StrictHostKeyChecking=no`) for rebuild convenience. That is safe on a trusted internal network, and unsafe on the public internet.
- Every project opens one or more NodePorts (SSH plus optional `web.ports`/`sidecar` ports) reachable through *any* node, and `web.ports` Ingress hosts are served without authentication or TLS by default. There are no `NetworkPolicy` objects or IP allowlists in the rendered manifests, so network isolation depends entirely on your cluster firewall/VPN.
- The key pair lives in `~/.config/coding-container/keys/` (outside the repo) and is shared by all projects by design; `ssh.keyDir` in a project config opts a project into its own key. The global config is machine-local; the project `config.yaml` is the only config that belongs in version control.
