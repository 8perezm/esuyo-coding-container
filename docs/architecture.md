...
The team's shared image state lives **in the cluster**, not on laptops: one ConfigMap, `coding-system` (label `coding-container=true`), in `k8s.namespace` (default `coding`). Its `data` block (all strings — ConfigMaps carry strings) holds the version pointer and policy:
