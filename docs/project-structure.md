...
├── Dockerfile              # Packaged starting point: `setup` copies it to ~/.config/esuyo-coding-container/; the build uses that personal copy
...
└── src/
    ...
    │   ├── deploy.js       # tag float (resolveDeployTag) + namespace + pull secret + manifest apply + rollout wait + SSH alias in ~/.ssh/config
    ...
    │   ├── ssh.js          # port-forward (default, auto port fallback) or direct node; per-project host keys
      │   ├── delete.js       # delete deployment/service/ingress/configmap (--keep-secrets) + remove SSH alias
    ...
    └── utils/
        ...
        ├── manifest.js     # ConfigMap/Deployment/Service/Ingress renderers + web.ports normalization + k8s.sidecars pass-through + nfs.volumes mounts + multi-doc YAML
...
- **Config is layered.** The project `config.yaml` (what a repo commits) overrides the global config at `~/.config/esuyo-coding-container/config.yaml`, which overrides built-in defaults; CLI flags win over all of it. Every relative path in a layer (`dockerfile`, `context`, `keyDir`) resolves against that layer's own directory, so the CLI works from any working directory.
- **SSH material lives globally, not in the repo.** `~/.config/esuyo-coding-container/keys/` holds the shared ed25519 key pair plus per-project `<project>-known_hosts` files; the first `deploy`/`create`/`ssh`/`key` invocation generates the pair. `coding-container key --force` rotates it (all deployed projects then need a `deploy`). A project can opt into its own key with `ssh.keyDir` in its config.
- **The build's Dockerfile lives globally too.** `setup` copies the repo's `Dockerfile` into `~/.config/esuyo-coding-container/` (plus a `.dockerignore`), and the global config points there — so the build context is the global folder, never the repo, and the private keys stay out of it.
